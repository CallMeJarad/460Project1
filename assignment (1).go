...
func robotRunLoop(lightSensorLeft *aio.GroveLightSensorDriver, lightSensorRight *aio.GroveLightSensorDriver, gopigo3 *g.Driver) {
	...
	for {
		...
		if sensorValLeft >= 2800 && sensorValRight >= 2800 {
		...
		if (sensorValLeft < 1150 && sensorValLeft > minLight) && (sensorValRight < 1150 && sensorValRight > minLight) {
			//fmt.Println(g.GetBatteryVoltage())
			gopigo3.SetMotorDps(g.MOTOR_LEFT, -50)
			gopigo3.SetMotorDps(g.MOTOR_RIGHT, 50)
			listValues(lightSensorLeft, lightSensorRight)
			fmt.Print("Sensors below 1250")

		} else if sensorValLeft < 2800 && sensorValRight < 2800 {
			listValues(lightSensorLeft, lightSensorRight)
			...
			if (sensorValLeft - sensorValRight) > sensorRange {
				...
				gopigo3.SetMotorDps(g.MOTOR_RIGHT+g.MOTOR_LEFT, 120)
